...
int main()
{
                ...
                for (l=0; l<num_wire_layers - 1 ; ++l)     // 
                {
                    ...
                    for (int j=0; j<num_neurons_current_layer_out; ++j) // 30
                    {
                        ...
                        for (int i=0; i<num_neurons_current_layer_in; ++i)//256
                        {
                            ...
                            LWEscheme->EvalAddEq(multi_sum[j],ctprep); //加了256次
                        }
                        cc.Decrypt(sk, multi_sum[j], &result3,space_msg);//这里先对sk模切换到ct->mod了
                    }
                }//end Layer
    ...
    if (statistics)
    {
        ...
        cout << "Disagreements: " << count_disagreements;  //clear和自举不一样
